    ...
    <body>
                    ...
                    <ul class="user-menu">
                        <li class="dropdown pull-right">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li id="logout"><a href="/SignUpSys/index.php/Admin/CTree/logout"><span class="glyphicon glyphicon-log-out"></span> Logout</a></li>
                            </ul>
                        </li>
                    </ul>
                </div>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">		
            ...
            <div class="ztree_right">
                <div>
                    <label>设置应该专业分流的年级</label>
                    <input type="text" class="form-control" id="input_rule_grade" placeholder="<?php echo $rule_grade;?>" />
                    <button class="btn btn-info" id="bt_rule_change">确定</button>
                </div>
        ...
        <script>
            ...
            var setting = {
                ...
                async:{
                    enable:true,
                    dataType:"json",
                    url:"/SignUpSys/index.php/Admin/CTree/getTree"
                },
                view: {
                    selectedMulti: false,
                    nameIsHTML: true
...
//                         trigger:'click',
//                         content : "人数上限："+treeNode.ctr_Limit+"  课程介绍："+treeNode.ctr_Synopsis,
//                         animation: true,
...
//            }
            function zTreeOnClick(event, treeId, treeNode) {
                ...
                if(treeNode.level<3){
                    ...
                    $('#leabel_synopsis').hide();
                }else if(treeNode.level === 3 ){
                    limit = treeNode.ctr_Limit;
                    $('#input_course_limit').show();
                    $('#label_limit').show();
                    $('#input_course_synopsis').show();
                    $('#leabel_synopsis').show();
                }else{
                    limit = treeNode.getParentNode().ctr_Limit;
                    $('#input_course_limit').hide();
                    $('#label_limit').hide();
                    $('#input_course_synopsis').show();
                    $('#leabel_synopsis').show();
                ...
                $('#input_course_limit').val(treeNode.ctr_Limit);
                $('#input_course_synopsis').val(treeNode.ctr_Synopsis);
            ...
            function add() {
                ...
                $.get("/SignUpSys/index.php/Admin/CTree/addChild",{pId:treeNode.ctr_NodeId,level:treeNode.level},function(data){
                    if(data === '0'){
                        alert("数据错误");
                    }
                    refresh();
            ...
            function remove(){
                ...
                $.get("/SignUpSys/index.php/Admin/CTree/removeChild",{ctr_NodeId:treeNode.ctr_NodeId},function(data){
                    if(data === 0){
                        alert("数据错误");
                    }
                    refresh();
                ...
                if(nodes.length === 0){
                    ...
                    if(treeNode.level <= 2){
                        if(nodeName.length === 0 || nodeSynopsis.length > 0){
                            ...
                            $.get("/SignUpSys/index.php/Admin/CTree/saveInfo",
                            {ctr_NodeId:treeNode.ctr_NodeId,
                                ctr_Name:nodeName},
                            function(data){
                                if(data === '0'){
                                    alert("数据错误");
            ...
            function setRule(){
                var grade = $('#input_rule_grade').val();
                if(!isNaN(grade) && grade.length!==0){
                    $.get("/SignUpSys/index.php/Admin/CTree/setRule",{ruleGrade:grade},function(data){
                        if(data == '1'){
                            $('#input_rule_grade').val("")
            ...
            $(document).ready(function(){
                ...
                $('#bt_rule_change').bind("click",setRule);
            });
            $('#logout').click(
                function(){
                    $.get("/SignUpSys/index.php/Admin/CTree/logout",function(){
                        window.location.reload();
                    });
                });
        </script>
